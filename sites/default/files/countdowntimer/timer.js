    ...
    Drupal.countdowntimer.formats = ['%days%D:%hours%H:%mins%M:%secs%S','<table class="countdown"><tr class="number"><td>%days%</td><td>%hours%</td><td>%mins%</td><td>%secs%</td></tr><tr class="label"><td>Days</td><td>Hours</td><td>Minutes</td><td>Seconds</td></tr></table>', '%days% shopping days left', '%days%:%hours%:%mins%:%secs%'];

    Drupal.countdowntimer.jstimer = function(timer_element) {
      /* defaults */
      var d = {dir:"down",format_num:0, format_txt:"", timer_complete:new String('Event started'), highlight:new String('class="red"').split(/=/), threshold:new Number('1440')};
      /* jstimer.properties */
